...
En esta página se describen todas las mejoras de Agile realizadas con la versión 23.1 en el entorno de vista previa. Estas mejoras estarán disponibles en el entorno de producción la semana del 16 de enero de 2023.
...
Las nuevas funciones de planificación de Scrum en los tableros de Adobe Workfront ofrecen opciones flexibles para administrar los procesos de Agile. Con estas herramientas, puede hacer lo siguiente:

* Realizar un seguimiento del trabajo en iteraciones o sprints
* Usar la velocidad para guiar los compromisos del equipo
* Rastrear la evolución y tasa de finalización
...
Cuando se añade una tarjeta conectada a un tablero para una tarea de Workfront, las subtareas se importan como elementos de lista de comprobación en la tarjeta. Además, cuando se crea un elemento de lista de comprobación en una tarjeta conectada, se añade una subtarea a la tarea de Workfront. Los elementos de la lista de comprobación de problemas no están conectados a ningún objeto de Workfront.

Anteriormente, los elementos de la lista de comprobación y las subtareas no estaban vinculados. Si desea incluir una subtarea en el tablero, puede importarla como una tarjeta conectada por separado o añadir manualmente un elemento de lista de comprobación a una tarjeta.
...
Hay disponible un nuevo ajuste de configuración para activar un contador de tarjetas para todas las columnas de un tablero. Si utiliza el límite de trabajo en curso en una columna, no se añade un contador de tarjetas independiente.
...
## Buscar y ordenar en el panel de control de tableros

>[!NOTE]
>
>Esta función solo está disponible a través de la inclusión de funciones anticipada para tableros de Workfront.

Ahora puede ordenar el panel de control de tableros por nombre o fecha del tablero y buscar un tablero específico en la lista.

Para obtener más información, consulte [Usar el panel de control de tableros](/help/quicksilver/agile/get-started-with-boards/use-boards-page.md).
...
Si se asigna un estado a una tarjeta de un tablero, el estado se muestra ahora en la tarjeta para que no tenga que abrirla para ver el estado. Esta mejora se aplica tanto a tarjetas ad hoc como a tarjetas conectadas.
...
Ahora puede enviar un vínculo a una tarjeta específica a otro usuario de tableros. La persona debe tener acceso para ver el panel antes de poder abrir el vínculo.

Cuando abre una tarjeta en un panel, la dirección URL del explorador tiene este aspecto:

```
https://<Workfront-URL>/boards/<board-id>/card/<card-id>. 
```

Puede copiar la dirección URL completa y enviarla a otra persona. Irá directamente a la tarjeta abierta cuando acceda al vínculo.

Anteriormente, los vínculos estaban disponibles para paneles, pero no para tarjetas específicas.

Para obtener información sobre las tarjetas, consulte [Añadir una tarjeta ad hoc a un panel](/help/quicksilver/agile/get-started-with-boards/add-card-to-board.md) y [Usar tarjetas conectadas en los paneles](/help/quicksilver/agile/get-started-with-boards/connected-cards.md).

## Filtrar por conexión en Paneles

>[!NOTE]
>
>Esta función solo está disponible a través de la inclusión de funciones anticipada para tableros de Workfront.

La lista de filtros de un panel ahora incluye la opción de filtrar por conexión, que muestra todas las tarjetas conectadas para un proyecto específico. También puede filtrar por tarjetas que no estén conectadas.
