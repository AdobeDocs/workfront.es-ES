...
Puede agregar una columna a una vista de tareas para mostrar una lista de las sucesoras de las tareas. El **Sucesoras de tareas** incluye el número de la sucesora y el nombre.
...
1. Elimine todo el texto del cuadro Modo de texto y sustitúyalo por el siguiente código:
   ...
   <pre>displayname=Sucesoras de tareas<br>listdelimiter=<br><br>listmethod=nested(sucesors).lists<br>textmode=true<br>type=iterate<br>valueexpression=CONCAT({sucessor}).{taskNumber},' - ',{sucessor}.{name})<br>valueformat=HTML</pre>
