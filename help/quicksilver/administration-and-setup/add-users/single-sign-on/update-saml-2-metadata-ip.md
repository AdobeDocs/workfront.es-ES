...
1. En el panel izquierdo, expanda **ADFS 2.0,** luego expandir **Relaciones de confianza.**

1. Haga clic en **Confianza de usuario fiable** carpeta.
1. Seleccione la confianza de usuario autenticado que configuró anteriormente para su uso con Workfront y, a continuación, en el panel derecho, haga clic en **Actualizar desde metadatos de federación**.
1. (Condicional) Si esta opción está atenuada (lo que significa que la confianza de usuario autenticado se configuró previamente mediante un archivo de metadatos), complete lo siguiente.
   ...
   1. Clic **Editar configuración.**
   1. Clic **Editar configuración**, luego seleccione **SAML 2.0** en el **Tipo** lista desplegable.

   1. Copie el **URL de metadatos**, que debería ser similar a la siguiente:

      `https://<yourdomain>.my.workfront.com/sso/downloadSAML2MetaData`

   1. En el servidor de ADFS, haga clic con el botón secundario en la confianza de usuario autenticado que configuró anteriormente y, a continuación, haga clic en **Propiedades.**
   1. Haga clic en **Monitorización** y pegue la dirección URL que ha copiado de Workfront en la pestaña **URL de metadatos de federación de usuario de confianza** field.

   1. Marque las opciones para **Supervisar usuario de confianza** y **Actualizar automáticamente el usuario de confianza**.

   1. Clic **OK.**
   1. Seleccione la confianza de usuario autenticado que configuró anteriormente para su uso con Workfront; a continuación, en el panel derecho, haga clic en **Actualización de metadatos de federación.**

1. Clic **OK** para omitir el mensaje sobre algunos de los metadatos de federación que no son compatibles con ADFS 2.0.
1. Abrir **Módulos De Windows Powershell.**
...
1. Para comprobar que esto funciona correctamente, utilice el **Visor de eventos** para buscar la siguiente información en los registros ADFS2.0:
...
1. Inicie sesión en el servidor de ADFS y abra **Consola de administración de ADFS**.
...
1. Seleccione la confianza de usuario autenticado que configuró anteriormente para su uso con Workfront y, a continuación, en el panel derecho, haga clic en **Actualizar desde metadatos de federación**.
   ...
   1. En Workfront, en el área Configuración, copie el **URL de metadatos** desde la pantalla de configuración de inicio de sesión único de Workfront.

      Para acceder a la información de **URL de metadatos**:

      1. Clic **Configurar** situado cerca de la esquina superior derecha de Adobe Workfront en la barra de navegación global.
      1. Haga clic > **Sistema** > **Inicio de sesión único (SSO)**.
      1. Clic **Editar configuración.**
      1. Clic **Editar configuración**, luego seleccione **SAML 2.0** en el **Tipo** lista desplegable.
      1. Copie el **URL de metadatos**, que debería ser similar a la siguiente:
   ...
   1. Marque las opciones para **Supervisar usuario de confianza** y **Actualizar automáticamente el usuario de confianza**.
   1. Clic **OK**.
   1. Seleccione la confianza de usuario autenticado que configuró anteriormente para su uso con Workfront y, a continuación, en el panel derecho, haga clic en **Actualización de metadatos de federación.**

1. Clic **OK** para omitir el mensaje sobre algunos de los metadatos de federación que no son compatibles con ADFS 2.0.
1. Clic **Actualizar** para completar la actualización de los metadatos de federación.

Los usuarios a los que se les permite acceder a Workfront a través de la pantalla nativa de inicio de sesión con las credenciales de inicio de sesión de Workfront (esto se puede configurar desde la página de perfil de cada usuario en la **Acceso** ) pueden iniciar sesión con su nombre de usuario y contraseña de Workfront navegando a la siguiente URL: `https://<yourdomain>.my.workfront.com/Workfront/login.cmd`.
...
Para obtener más información sobre cómo obtener una nueva URL de metadatos de Workfront, consulte [Actualizar los metadatos de ADFS](#update-your-adfs-metadata).
